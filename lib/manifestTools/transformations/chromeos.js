...
function convertFromBase(manifestInfo, callback) {
  ...
  var manifest = {
    'manifest_version': 2,
    'name': originalManifest.name 
            || originalManifest.short_name
            || utils.sanitizeName(url.parse(originalManifest.start_url).hostname),
    'version': '0.0.1',
    'app': {
      'launch': {
  ...
  // map extended scope
  if (originalManifest.mjs_extended_scope && originalManifest.mjs_extended_scope.length) {
    manifest.app.urls = originalManifest.mjs_extended_scope;
  }
