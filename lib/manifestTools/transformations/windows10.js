...
function convertFromBase(manifestInfo, callback) {
  ...
  fs.readFile(manifestTemplatePath, function (err, data) {
    ...
    rawManifest = rawManifest.replace(/{IdentityName}/g, guid)
                             ...
                             .replace(/{ApplicationId}/g, originalManifest.short_name)
                             .replace(/{StartPage}/g, originalManifest.start_url)
                             .replace(/{DisplayName}/g, originalManifest.short_name)
                             .replace(/{Description}/g, originalManifest.name || originalManifest.short_name)
                             .replace(/{RotationPreference}/g, originalManifest.orientation || 'portrait');
      ...
      for (var i = 0; i < originalManifest.icons.length; i++) {
        ...
        if (isValidIconFormat(icon, validIconFormats)) {
          ...
          if (iconDimensions[0] === '30' && iconDimensions[1] === '30') {
            icons['30x30'] = { 'url': icon.src, 'fileName': 'smalllogo.scale-100.png' };
