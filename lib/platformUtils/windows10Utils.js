...
//var domain = 'http://localhost:1494';
var domain = 'http://cloudappx.azurewebsites.net';

var buildPackage = function (appName, appFolder, callback) {
  ...
  cloudappx.makeappx(options).then(
    ...
    function (err) {
      console.log('Unable to create the package locally. Invoking the CloudAppX service instead...');
      makeAppx(appName, appFolder, function (err) {
...
function makeAppx(appName, appFolder, callback) {
  ...
  output.on('close', function () {
    var req = request.post(domain + '/v1/upload', function (err, resp, body) {
      ...
      fs.unlink(zipFile, function (err) {
        ...
        var req = request.get(domain + '/' + body)
