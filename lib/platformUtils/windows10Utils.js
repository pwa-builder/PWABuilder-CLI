...
function replaceManifestValues(w3cManifest, content, timestamp) {
  var replacedContent = content;
  var guid = utils.newGuid();
  var generationDate = timestamp || new Date().toISOString().replace(/T/, ' ').replace(/\.[0-9]+/, ' ');
  ...
  replacedContent = replacedContent.replace(/{IdentityName}/g, guid)
                                    ...
                                    .replace(/{GeneratedFrom}/g, undefined || 'JSON Manifest') //Website Wizard
                                    .replace(/{GenerationDate}/g, generationDate);

  // Add additional metadata
  if (true) {    
    replacedContent = replacedContent.replace(/{MetadataItems}/g, '');
    //'<build:Item Name ="' + '' + '" Version ="' + '' + '" />';
  }
