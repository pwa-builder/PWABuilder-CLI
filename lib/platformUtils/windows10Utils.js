  ...
  fs.readFile(manifestPath, 'utf8', function (err, data) {
    if (!err) {
      ...
      if (packageIdentityPlaceholders.test(data) || publisherDisplayNamePlaceholder.test(data)) {
        err = new Error('You must register the app in the Windows Store and obtain the Package/Identity/Name, Package/Identity/Publisher, and Package/Properties/PublisherDisplayName details. Update the placeholders in the appmanifest.xml file with this information before creating the App Store package.');
      }
    }
...
var makeAppx = function (appFolder, outputPath, callback) {
  ...
  validateManifestPublisherDetails(appFolder, function (err, result) {
    if (err) {
      return callback && callback(err);
    }
