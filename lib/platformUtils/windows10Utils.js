...
function invokeCloudAppX(appName, appFolder, callback) {
  ...
  output.on('close', function () {
    ...
    log.debug('Invoking CloudAppX service...');
    ...
    var req = request.post(options, function (err, resp, body) {
      ...
      if (resp.statusCode != 200) {
        return callback && callback(new Error('Failed to create the package. The CloudAppX service returned an error: ' + resp.statusCode + ' - ' + resp.statusMessage + ': \'' + body + '\''));
      }

      var appxFilepath = path.join(appFolder, appName + '.appx');
      fs.writeFile(appxFilepath, body, { "encoding": 'binary' }, function (err) {
        if (err) {
          return callback && callback(err);
        }
