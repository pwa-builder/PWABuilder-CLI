...
var runApp = function (platform, callback) {
  ...
  if (platform.toUpperCase() === 'WINDOWS10') {
    if (!isWindows()) {
...
var openVisualStudio = function (callback) {
  ...
  var windows81SolutionFileName = 'CordovaApp.sln';
  ...
  searchFile(process.cwd(), windows10ProjectFileName, function (err, results) {
    ...
    getWindowsVersion(function (error, version) {
      ...
      if (results && results.length > 0 && isWindows10Version(version)) {
        ...
        searchFile(process.cwd(), windows81SolutionFileName, function (err, results) {
          if (err) {
            log.debug(err);
            return callback(new Error('Failed to find the Visual Studio solution: "' + windows81SolutionFileName + '"'));
          }
