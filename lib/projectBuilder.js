  ...
  exec(cmdLine, function (err, stdout, stderr) {
    ...
    manifestTools.writeToFile(w3cManifestInfo, manifestFilePath, function (err) {
      ...
      exec(cmdLine, function (err, stdout, stderr) {
        ...
        var somePendingTaskFailed = false;
        platforms.forEach(function (platform) {
          pendingTasks = pendingTasks.fail(function () { somePendingTaskFailed = true; })
                                     .fin(function () { return createCordovaAppForPlatform(generatedAppDir, platformDir, build, platform); });
        });

        pendingTasks.done(function () {
          // set original working directory as current (so that the generated folder can be deleted)
          process.chdir(originalPath);
          if (somePendingTaskFailed) {  
