...
var updateProjectFiles = function (sourceDir, icons, callback) {
  var packageManifestPath = path.join(sourceDir, 'package.appxmanifest');
  fileUtils.replaceFileContent(packageManifestPath,
    function (data) {
      // TODO: replace logos in package.appxmanifest
      return data;
              // .replace(/{StoreLogo}/g, path.join('images', 'storelogo.scale-100.png'))
              // .replace(/{Logo}/g, path.join('images', 'logo.scale-100.png'))
              // .replace(/{SmallLogo}/g, path.join('images', 'smalllogo.scale-100.png'))
              // .replace(/{SplashScreen}/g, path.join('images', 'splashscreen.scale-100.png'));
    },
    function (err) {
      if (err) {
      ...
      var projectFilePath = path.join(sourceDir, 'App.jsproj');
      fileUtils.replaceFileContent(projectFilePath,
        function (data) {
          // TODO: replace logos in App.prog
          return data;
          // .replace(/{StoreLogo}/g, path.join('images', 'storelogo.scale-100.png'))
          // .replace(/{Logo}/g, path.join('images', 'logo.scale-100.png'))
          // .replace(/{SmallLogo}/g, path.join('images', 'smalllogo.scale-100.png'))
        ...
        function (err) {
          if (err) {
            log.warn('WARNING: Failed replace values of App.jsproj');
            log.debug(err);
            return callback(err);
          }
...
var createWindowsApp = function (w3cManifestInfo, generatedAppDir) {
  ...
  manifestTools.convertTo(w3cManifestInfo, 'windows10', function (err, windowsManifestInfo) {
    ...
    var projectAssetsDir = path.join(__dirname, 'projectBuilder', 'assets', 'windows');
    var projectAssetsImagesDir = path.join(projectAssetsDir, 'images');

    // if the platform icons dir doesn't exist, create it
    mkdirp(manifestImagesDir, function (err) {
      ...
      log.info('Copying Windows source project assets...');
      ncp(projectAssetsDir, sourceDir, function (err) {
        ...
        function downloadIcon(size) {
          ...
          pendingDownloads.push(downloadTask.promise);
          var iconUrl = url.resolve(w3cManifestInfo.content.start_url, icons[size]);
          downloader.downloadImage(iconUrl, manifestImagesDir, function (err, data) {
            if (err) {
              log.warn('WARNING: Failed to download icon file: ' + icons[size] + ' (' + err.message + ')');
              log.debug(err);
            } else {
              ...
              // Change icons reference to use the local path
              windowsManifestInfo.content.rawData = windowsManifestInfo.content.rawData.replace(new RegExp(icons[size], 'g'), path.join('images', localPath));

              // Add default logos (where necessary)
              windowsManifestInfo.content.rawData = windowsManifestInfo.content.rawData.replace(/{StoreLogo}/g, path.join('images', 'storelogo.scale-100.png'));
              windowsManifestInfo.content.rawData = windowsManifestInfo.content.rawData.replace(/{Logo}/g, path.join('images', 'logo.scale-100.png'));
              windowsManifestInfo.content.rawData = windowsManifestInfo.content.rawData.replace(/{SmallLogo}/g, path.join('images', 'smalllogo.scale-100.png'));
              windowsManifestInfo.content.rawData = windowsManifestInfo.content.rawData.replace(/{SplashScreen}/g, path.join('images', 'splashscreen.scale-100.png'));
            }
          ...
          fs.writeFile(manifestFilePath, windowsManifestInfo.content.rawData, function (err) {
            ...
            ncp(projectAssetsImagesDir, manifestImagesDir, { clobber: false }, function (err) {
              ...
              ncp(manifestImagesDir, sourceImagesDir, { clobber: true }, function (err) {
                ...
                updateProjectFiles(sourceDir, icons, function (){
                  if (err) {
                    log.error(err.message);
