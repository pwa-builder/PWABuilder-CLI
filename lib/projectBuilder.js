...
var copyDefaultChromeIcon = function(chromeManifest, targetPath, callback) {
  if (chromeManifest.icons) {
    callback(undefined);
  } else {
    log.info('Copying default icon for Chrome...');
    
    var iconFilename = 'icon128.png';
    var source = path.join(__dirname, 'projectBuilder', 'assets', 'chrome', 'images', iconFilename);
...
var copyDefaultFirefoxIcon = function(firefoxManifest, targetPath, callback) {
  if (firefoxManifest.icons) {
    callback(undefined);
  } else {
    log.info('Copying default icon for Firefox...');
    
    var iconFilename = 'icon128.png';
    var source = path.join(__dirname, 'projectBuilder', 'assets', 'firefox', 'images', iconFilename);
    var target = path.join(targetPath, iconFilename);
...
var createFirefoxApp = function (w3cManifestInfo, generatedAppDir) {
  ...
  manifestTools.convertTo(w3cManifestInfo, 'firefox', function (err, firefoxManifestInfo) {
    ...
    mkdirp(platformDir, function (err) {
      ...
      Q.allSettled(pendingDownloads)
        .done(function () {
        copyDefaultChromeIcon(firefoxManifestInfo.content, platformDir, function (err) {
          if (err) {
            log.error('WARNING: Failed to copy the default icon for Chrome.');
            log.debug(err);
          } 
      ...
      .done(function (results) {
        ...
        results.forEach(function (task) {
          if (task.state !== 'fulfilled') {
            log.error('ERROR: ' + task.reason.message);
            innerErrors.push(task.reason);
          }
        });

        callback && callback(innerErrors.length ? wrapError(errmsg, innerErrors) : null);
      });
    });
  });
};
