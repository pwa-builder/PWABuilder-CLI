...
var createApps = function (w3cManifestInfo, rootDir, platforms, options, callback) {
  ...
  manifestTools.validateManifest(w3cManifestInfo, platforms, function (err, validationResults) {
    ...
    validationResults.forEach(function (validationResult) {
      var validationMessage = validationResult.level.toUpperCase() +  ': Manifest validation ' +  ' (' + validationConstants.platformDisplayNames[validationResult.platform] + ') - ' + validationResult.description + '[' + validationResult.member + '] - Code: ' + validationResult.code;
    ...
    var appName = utils.sanitizeName(w3cManifestInfo.content.short_name);
