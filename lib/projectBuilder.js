...
function createApps(manifestInfo, rootDir, platforms, build, callback) {
  ...
  mkdirp(generatedAppDir, function (err) {
    ...
    platforms.forEach(function (el) {
      ...
      if (platform === 'FIREFOX') {
        createFirefoxApp(manifestInfo, generatedAppDir, build, callback);
      ...
      createCordovaApp(manifestInfo, generatedAppDir, cordovaPlatforms, build, function (err) {
        if (!err) {
          cordovaPlatforms.forEach(function (platform) {
            createPlatformShortcut(generatedAppDir, platform);
          });
        }
    ...
    mkdirp(platformDir, function (err) {
      ...
      for (var size in icons) {
        ...
        downloader.downloadImage(icons[size], platformDir, function (err, data) {
          if (err) {
            log.warn('WARNING: Failed to download icon file at \'' + icons[size] + '\'.');
            log.debug(err);
          } else {
...
function createFirefoxApp(manifestInfo, generatedAppDir, build, callback) {
  log.info('Generating the Firefox application...');
  
  manifestTools.convertTo(manifestInfo, 'firefox', function (err, manifestInfo) {
    if (err) {
      log.error(err.message);
      return;
    }
...
function createPlatformShortcut(projectPath, platform) {
  ...
  fs.symlink(srcpath, dstpath, 'junction', function (err) {
    if (err) {
      log.error('An error occurred creating a shortcut to the \'' + platform + '\' platform.');
      return;
    }
    
    log.info('The shortcut to the \'' + platform + '\' platform was created successfully.');
  });
}
