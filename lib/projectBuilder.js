    ...
    mkdirp(platformDir, function (err) {
      ...
      Q.allSettled(pendingDownloads)
        ...
        copyDefaultChromeIcon(chromeManifestInfo.content, platformDir, function (err) {
          if (err) {
            log.error('WARNING: Failed to copy the default icon for Chrome.');
            log.debug(err);
          }          
            ...
            ncp(projectAssetsDir, sourceDir, function (err) {
              ...
              ncp(manifestImagesDir, sourceImagesDir, { clobber: true }, function (err) {
                ...
                updateProjectFiles(sourceDir, w3cManifestInfo.content, function () {
                  if (err) {
                    return task.reject(wrapError('ERROR: The Windows 10 project files could not be updated.', err));
                  }

                  copyDocFile('Windows10-next-steps.md', platformDir, function (err) {
                    if (err) {
                      log.error('WARNING: Failed to copy documentation file to the Windows 10 platform folder.');
                      log.debug(err);
                    }
...
var createCordovaApp = function (w3cManifestInfo, generatedAppDir, platforms, build) {
  ...
  exec(cmdLine, function (err) {
    ...
    if (err) {
      return task.reject(wrapError('ERROR: Failed to create the Cordova application. The Cordova project could not be created successfully.', err));
    }
    ...
    manifestTools.writeToFile(w3cManifestInfo, manifestFilePath, function (err) {
      ...
      exec(cmdLine, function (err) {
        ...
        if (err) {
          return task.reject(wrapError('ERROR: Failed to add the Hosted Web App plugin. The Cordova project could not be created successfully.', err));
        }
        ...
        platforms.forEach(function (platform) {
          ...
          exec(cmdLine, function (err) {
            ...
            if (err) {
              return platformTask.reject(wrapError('WARNING: Failed to add ' + mapToCordovaPlatform(platform) + ' platform.', err));
            }
            ...
            applyPlatformsFix(platform, platformDir, function (err) {
              ...
              copyDocFile(getCordovaDocFilename(platform), docTargetPath, function (err) {
                if (err) {
                  log.error('WARNING: Failed to copy documentation file to the Cordova platform folder.');
                  log.debug(err);
                }
        ...
        Q.allSettled(pendingTasks)
          .done(function (results) {
            if (results.some(function (platformTask) { return platformTask.state !== 'fulfilled'; })) {
              return task.reject(wrapError('One or more tasks failed while generating the Cordova application.'));
            } else {
              return task.resolve();
            }
          });
