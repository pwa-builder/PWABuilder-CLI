...
var pluginIdOrUrl = 'cordova-plugin-hostedwebapp@">=0.1.0 <0.2.0"';
            ...
            ncp(projectAssetsDir, sourceDir, function (err) {
              ...
              ncp(manifestImagesDir, sourceImagesDir, { clobber: true }, function (err) {
                ...
                updateProjectFiles(sourceDir, w3cManifestInfo.content, function (err) {
                  if (err) {
                    return task.reject(wrapError('ERROR: Failed to update Windows 10 pac¿kage.appxmanifest. The Windows 10 project files could not be created successfully.', err));
                  }
                  ...
                  copyDocFile('Windows10-next-steps.md', platformDir, function (err) {
                    ...
                    if (options.build) {
                      ...
                      windows10Utils.makeAppx(options.appName, platformDir, function (err) {
                        if (err) {
                          return task.reject(wrapError('ERROR: Failed to create a Windows Store AppX package for the Windows 10 hosted app project.', err));
                        }
      ...
      exec(cmdLine, function (err) {
        ...
        exec(cmdLine, function (err) {
          ...
          platforms.forEach(function (platform) {
            ...
            var docTargetPath = path.join(platformDir, 'platforms', platform);
