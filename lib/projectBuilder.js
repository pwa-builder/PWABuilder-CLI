...
var pluginIdOrUrl = require('com-manifoldjs-hostedwebapp'); //'https://github.com/manifoldjs/ManifoldCordova.git';
...
var createFirefoxOSApp = function (w3cManifestInfo, generatedAppDir) {
  ...
  manifestTools.convertTo(w3cManifestInfo, 'firefox', function (err, firefoxManifestInfo) {
    if (err) {
      log.error(err.message);
      return task.reject(new Error('The Firefox OS project could not be created successfully.'));
    }
    ...
    mkdirp(platformDir, function (err) {
      ...
      manifestTools.writeToFile(firefoxManifestInfo, manifestFilePath, function (err) {
        ...
        copyDocFile('Firefox-next-steps.md', platformDir, function (err) {
          if (err) {
            log.error('WARNING: Failed to copy documentation file to the Firefox OS platform folder.');
            log.debug(err);
          }
...
var createCordovaAppForPlatform = function (generatedAppDir, platformDir, build, platform) {
  log.info('Adding Cordova platform: ' + mapToCordovaPlatform(platform) + '...');
  
  var platformTask = Q.defer();
  
  if (platform === 'windows81') {
    log.warn('Forced fail to generate windows81 platform.');
    platformTask.reject(new Error('Error'));
    return platformTask.promise;
  }

  var cmdLine = cordovaPath + ' platform add ' + mapToCordovaPlatform(platform);
  log.debug('    ' + cmdLine);
  ...
  exec(cmdLine, function (err, stdout, stderr) {
    ...
    manifestTools.writeToFile(w3cManifestInfo, manifestFilePath, function (err) {
      ...
      exec(cmdLine, function (err, stdout, stderr) {
        ...
        var pendingTasks = new Q();
        platforms.forEach(function (platform) {
          pendingTasks = pendingTasks.fin(function () { return createCordovaAppForPlatform(generatedAppDir, platformDir, build, platform); });
        });

        pendingTasks.done(function () {
        ...
        function () {
          // set original working directory as current (so that the generated folder can be deleted)
          process.chdir(originalPath);
          task.reject(new Error('One or more tasks failed while generating the Cordova application.'));
        });
      });
    });
  });
...
module.exports = {
  ...
  createFirefoxOSApp: createFirefoxOSApp,
