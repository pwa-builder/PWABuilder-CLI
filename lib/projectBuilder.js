...
var createWebApp = function (w3cManifestInfo, generatedAppDir) {
  ...
  mkdirp(platformDir, function (err) {
    function createDownloaderImageCallback(downloadTask, icon) {
      return function (err) {
        if (err) {
          log.warn('WARNING: Failed to download icon file: ' + icon + ' (' + err.message + ')');
          log.debug(err);
        }
    ...
    function createMkdirpCallback(iconPath, iconUrl, destinationFolder, downloadTask, icon) {
      return function (err) {
        if (err) {
          log.warn('WARNING: Failed to create icon folder: ' + iconPath + ' (' + err.message + ')');
          log.debug(err);
          return task.reject(new Error('Failed to create icon folder'));
        }

        downloader.downloadImage(iconUrl, destinationFolder, createDownloaderImageCallback(downloadTask, icon));
      };
    }
    ...
    for (var i = 0; i < icons.length; i++) {
      ...
      var iconPath = icons[i].src.replace(/[^\/]*$/, '');
      var destinationFolder = path.join(platformDir, iconPath);
      mkdirp(destinationFolder, createMkdirpCallback(iconPath, iconUrl, destinationFolder, downloadTask, icons[i]));
    }
