...
function validateManifest(manifestInfo, targetPlatforms) {
    if (!manifestInfo || !manifestInfo.content) {
        // new Error('Manifest content is empty or invalid.')
        return;
    }
    ...
    if (!targetPlatforms) {
      validationTargetPlatforms = ['general'];
    } else if (targetPlatforms.length === 0 || targetPlatforms.indexOf('general') < 0) {
      var validationTargetPlatforms = targetPlatforms.slice(); // Creates a copy
      validationTargetPlatforms.push('general');
    }

    var validationResults = [];

    for (var i = 0; i < validationTargetPlatforms.length; i++) {
      var validationFunc = validationRules[validationTargetPlatforms[i]];

      if (validationFunc) {
        var platformResults = validationFunc(manifestInfo.content);
