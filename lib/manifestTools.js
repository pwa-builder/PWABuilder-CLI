...
function validateManifest(manifestInfo, targetPlatforms, callback) {
    ...
    if (manifestInfo.format === c.BASE_MANIFEST_FORMAT) {
      ...
      var schemaValidation = tv4.validateMultiple(manifestInfo.content, schema, true, true);
      schemaValidation.errors.forEach(function (err) {
        ...
        if (err.code !== 1000 || (member && member.indexOf('_') < 0)) {
          validationResults.push({
            'description': message,
            'platform': validationConstants.platforms.all,
            'level': (err.code === 1000) ? validationConstants.levels.warning : validationConstants.levels.error,
            'member': err.dataPath,
            'code': 'w3c-schema-' + err.code
          });
        }
      });
    }
